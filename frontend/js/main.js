...
async function cargarServicios() {
  try {
    ...
    grid.innerHTML = servicios
      .map((s) => {
        if (s.activo === false) {
          let fechaTexto = "Próximamente disponible";
          if (s.disponibleDesde) {
            const fechaObjetivo = new Date(s.disponibleDesde);
            const diffMs = fechaObjetivo - new Date();
            const fechaLegible = formatearFecha(s.disponibleDesde);
            if (diffMs > 0) {
              const dias = Math.floor(diffMs / 86400000);
              const horas = Math.floor((diffMs % 86400000) / 3600000);
              const cuenta = dias >= 1 ? `en ${dias} día${dias > 1 ? "s" : ""}` : `en ${horas} hora${horas > 1 ? "s" : ""}`;
              fechaTexto = `Disponible ${cuenta} (${fechaLegible})`;
            } else {
              fechaTexto = "Disponible muy pronto";
            }
          }
          return `
